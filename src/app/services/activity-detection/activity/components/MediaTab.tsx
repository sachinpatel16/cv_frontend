...
export function MediaTab() {
  ...
  const isSmoking = selectedDetector === 'smoking';
  // Smoking is video-only; lock the media type for it
  const [localType, setLocalType] = useState<'photo' | 'video'>(
    isSmoking ? 'video' : mediaType,
  );
  ...
  return (
    <div className="space-y-5 rounded-xl border border-[#1E3048] bg-[#0D1628] p-5">
      ...
      {/* Media type toggle — hidden for smoking (video-only) */}
      {!isSmoking && (
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            ...
            <span className="rounded-md bg-[#1E3048] px-2 py-0.5 font-mono text-xs font-semibold text-[#E8EDF5]">
              {stepLabel(intervalToStep(interval))}
            </span>
          </div>
          <input
            type="range"
      ...
      >
        {uploading ? (
          ...
          <>
            ...
            {isSmoking ? 'Upload & Start Analysis' : 'Upload & Continue'}
          </>
        )}
      </button>
    </div>
